...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
	...
	// start logging code
	//u8* fnn = alloc_printf("%s/used_inputs", out_dir);
	//FILE *logptr = fopen(fnn, "a");
	fprintf(logptr, "%d ", fault);
	for (int i = 0; i < len; i++)
		fprintf(logptr, "%c", ((unsigned char *) mem)[i]);
	fprintf(logptr, "\n");
	//fclose(logptr);
	//end logging code
